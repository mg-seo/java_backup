...
public class StringQuiz02 {
	public static void main(String[] args) {
		 ...
		 */
		String num = "950101-1000000";
		try {
			System.out.println(masking(num));
			System.out.println(masking(num).charAt(7) == '1' ? "남자" : "여자");
		} catch (Exception e) {
	...
	private static String masking(String num) throws Exception{
		String temp = num.replace("-", "");
		if(temp.length() != 13 | (temp.charAt(6) != '1' && temp.charAt(6) != '2')) {
			throw new Exception();
		} else {
			num = temp.substring(0, 6);
			num += "-" + temp.charAt(6) + "******";
			return num;
		}
	}
